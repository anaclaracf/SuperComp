...
struct temp {
    ...
    int operator()(const int& x){
        ...
        int score = (letter_B == sequence_A[x-1] ? 2 : -1);
...
int main(){
    ...
    thrust::device_vector<int> maximos;
    ...
    for (int i=0; i < size_A; i++) {
        for (int j=0; j < size_B; j++) {
            ...
            thrust::counting_iterator<int> c1(size_subA+1);
            ...
            for(int e = 0; e < size_subB; e++){
                ...
                thrust::inclusive_scan(calc[1].begin()+1, calc[1].end(), calc[0].begin()+1, lateral());
            }

            maximos.push_back(calc[0].back());
