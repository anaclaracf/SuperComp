...
int max_score(matrix H, string first_seq, string second_seq, int n, int m, int w){
...
int main(){
    ...
    for (int i=0; i< int(sub_A.size());i++) {
        for (int j=0; j< int(sub_B.size());j++) {
            ...
            subsequencias.seqA = e1;
            subsequencias.seqB = e2;
            subsequencias.score = 0;

            sequencias.push_back(subsequencias);
    ...
    for(int i = 0; i < int(sequencias.size()); i++){
        ...
        int score;
        string e1 = sequencias[i].seqA;
        string e2 = sequencias[i].seqB;
        int tamanho_e1 = e1.size();
        int tamanho_e2 = e2.size();

        score = max_score(H, "-"+e1, "-"+e2, tamanho_e1, tamanho_e2, w);
        sequencias[i].score = score;
    }


    for(int i = 0; i < int(sequencias.size()); i++){
        if(sequencias[i].score >= max){
            max = sequencias[i].score;
            seqA_utilizada = sequencias[i].seqA;
            seqB_utilizada = sequencias[i].seqB;
        }
    }
